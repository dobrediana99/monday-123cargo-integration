...
import type { MondayClient, MondayColumnValue } from "../services/mondayClient.js";
...
export type BursaAuthResult = { ok: true; authHeader: string } | { ok: false; error: string };
 ...
 */
export async function resolveBasicAuthForBursa(
  monday: MondayClient,
  cols: Record<string, MondayColumnValue>
): Promise<BursaAuthResult> {
  const cfg = getConfig();
...
export function bursaResponseRequiresTwoStep(status: number, data: unknown): boolean {
  if (status === 409) return true;
  return String((data as { response?: string })?.response || "").toLowerCase().includes("2 step authentication required");
}
...
function extractTwoStepCookie(data: unknown, setCookieHeader: unknown): string | null {
  const d = data as Record<string, unknown>;
  const candidates: unknown[] = [
    d?.response,
    (d?.response as Record<string, unknown>)?.cookie,
    (d?.response as Record<string, unknown>)?.value,
    (d?.response as Record<string, unknown>)?.["Bursa-2step-authentication"],
...
async function publishLoad(context: IntegrationContext): Promise<IntegrationResult> {
  ...
  const ok = response.status === 200 && (response.data as { resultCode?: number })?.resultCode === 0;
...
async function completeTwoStepPublish(context: IntegrationContext, code: string): Promise<IntegrationResult> {
  ...
  const ok = publishRes.status === 200 && (publishRes.data as { resultCode?: number })?.resultCode === 0;
  if (ok) return { status: "success" };
